...
def parser(input_,SUBJECT_FILE,CONTRACT_FILE):
  ...
  sdf.to_excel('CRIFSUBJECTDATA_BPARSING.xlsx',index=None)
  cdf.to_excel('CRIFCONTRACTDATA_BPARSING.xlsx',index=None)
  ...
  cdf['Outstanding Payments Number']= pd.to_numeric(cdf['Outstanding Payments Number']).astype(int)
  cdf['Number of Payments']= pd.to_numeric(cdf['Number of Payments']).astype(int)
